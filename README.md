...
- **联机小游戏**：支持斗地主、象棋、五子棋、UNO、UNO No Mercy 五个联机游戏，独立房间管理，实时对战
  ...
  - **UNO No Mercy**：2-10人残酷对战，148张牌全新牌组，14种卡牌效果独立注册表架构，惩罚叠加、同色全弃、罚抽颜色、反转+4、全场跳过、25张淘汰制
...
│   │   ├── routes.py        # 游戏页面路由
│   │   ├── api.py           # 房间管理 REST API
│   │   ├── effects.py       # 14种卡牌效果注册表（handler模式）
│   │   ├── game.py          # 统一游戏操作（淘汰/回合/结束）
│   │   └── websocket.py     # Socket.IO 游戏事件路由
│   ├── main/                # 主页面模块
│   │   ├── __init__.py
│   │   └── routes.py        # 主页面路由
│   ├── settings/            # 系统设置模块
...
- **2026-05-29**：修复服务器时间处理不一致问题，统一使用 `datetime.now(timezone.utc)` 替代已弃用的 `datetime.utcnow()`，修复游戏模块时间戳无时区信息导致的显示错误
