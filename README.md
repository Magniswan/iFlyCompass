...
- **用户管理**：支持用户注册、登录、权限管理
  ...
  - **管理员开关用户**：管理员可禁用/启用用户账号，被禁用的用户无法登录
...
- **联机小游戏**：支持斗地主、象棋、五子棋、UNO、UNO No Mercy 五个联机游戏，独立房间管理，实时对战
  ...
  - **积分系统**：自动记录每局胜负积分，支持查看排行榜
  - **再来一局**：游戏结束后玩家可一键发起再来一局，快速开始下一局
...
- **UNO No Mercy**：2-10人残酷对战，全新牌组
  - 148张牌：数字牌 + 经典功能牌 + 同色全弃/全场跳过/反转+4/罚抽颜色/+6/+10
...
- **2026-06-08**：修复 UNO No Mercy 排名逻辑（区分出完手牌和被淘汰的排名，出完手牌的玩家名次靠前，被淘汰玩家名次靠后）；修复 UNO 系列万能牌颜色选择按钮不显示颜色的问题
- **2026-06-07**：修复 UNO No Mercy 回合推进Bug（打出0/7后下家被拒绝出牌）；修复切换/传递/全弃后空手牌未检测问题；修复同色全弃牌显示混乱；修复自动出牌效果缺失与CR罚抽竞态
- **2026-06-07**：修复小游戏大厅进入房间后不直接跳转房间内部的问题，现在创建或加入房间后自动进入游戏界面
- **2026-06-06**：为多款棋牌游戏添加积分系统与再来一局功能
- **2026-06-05**：移除所有本地 libs 目录下的第三方依赖包文件，精简项目结构
- **2026-06-04**：修复网页代理相关问题
- **2026-06-03**：B站视频新增转编码选项，支持用户自主选择是否转编码；修复用户无法更改密码的 bug
- **2026-06-02**：新增管理员开关用户选项（禁用/启用账号）；优化 UNO No Mercy 游戏逻辑
- **2026-05-30**：游戏大厅统一重构，新增统一创建房间功能与活跃房间展示；优化 mitmproxy 后台运行方式；优化 bat 启动脚本逻辑
- **2026-05-29**：mitmproxy 改为后台静默运行（无窗口），启动时自动关闭旧进程，停止时可靠终止所有 mitmdump 进程
- **2026-05-29**：修复 B站视频硬件编码器检测误判问题，NVENC/QSV 检测从仅检查编译支持改为实际初始化验证，增强运行时编码器错误识别模式；新增 UNO 和 UNO No Mercy 两款联机卡牌游戏；修复斗地主飞机牌型识别 bug；修复 12 个界面侧边栏小游戏入口缺失问题；修复同色全弃牌前端显示；手牌自动按颜色排序；修复万能牌颜色指示器不更新问题
- **2026-05-29**：修复服务器时间处理不一致问题，统一使用 `datetime.now(timezone.utc)` 替代已弃用的 `datetime.utcnow()`
